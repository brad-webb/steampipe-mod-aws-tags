variable "mandatory_tags_at_least_one" {
  type        = list(string)
  description = "A list of mandatory tags, at least one of which must exist."
  ...
  mandatory_sql = <<-EOQ
    with analysis as (
      select
        arn,
        title,
        tags ?| $1 as has_any_mandatory_tags,
    ...
    select
      arn as resource,
      case
        when has_any_mandatory_tags then 'ok'
        else 'alarm'
      end as status,
      case
        when has_any_mandatory_tags then title || ' has at least one of the mandatory tags.'
...
benchmark "at_least_one" {
  title       = "At least one"
  description = "Resources should all at least one of the set of tags. Use case is where a mandatory tag has had a change in format."
  children = [
...
control "accessanalyzer_analyzer_mandatory" {
  ...
  param "mandatory_tags_at_least_one" {
    default = var.mandatory_tags_at_least_one
  }
}
